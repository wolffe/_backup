...
switch($mode) {
	...
	case 'list':
		...
		$result = mysql_query('SELECT * FROM `miniblog` ORDER BY date DESC') or die(mysql_error());
		
		while($row = mysql_fetch_assoc($result)) {
			$published = ($row['published'] == 1) ? 'Published' : 'Unpublished';
			
			$preview_link = ($row['published'] == 1) ? "<a href=\"../{$config['miniblog-filename']}?post={$row['post_slug']}\"><i class=\"fa fa-eye fa-fw fa-2x\" title=\"View post\"></i></a>" : '';
			$post_list .= "<tr>
								...
								<td>
								{$preview_link}
                                <a href=\"admin.php?mode=edit&id={$row['post_id']}\"><i class=\"fa fa-pencil fa-fw fa-2x\" title=\"Edit post\"></i></a>
								<a href=\"admin.php?mode=delete&id={$row['post_id']}\" onclick=\"return confirm_dialog('admin.php?mode=delete&id={$row['post_id']}', 'This will remove the post, are you sure you want to continue?')\"><i class=\"fa fa-trash-o fa-fw fa-2x\" title=\"Delete post\"></i></a>
								</td>
							</tr>";
		}
	...
	case 'options':
        ...
        while($row = mysql_fetch_array($sql)) {
            $option_list .= '<p>
								<input type="text" name="data[' . $row['config_name'] . ']" value="' . stripslashes($row['config_value']) . '" id="' . $row['config_name'] . '">
								<label for="' . $row['config_name'] . '">' . str_replace('-', ' ', trim(ucfirst($row['config_name']))) . '</label>
                                <br><span class="form-text">' . $row['config_explain'] . '</span>
							</p>';
		}
