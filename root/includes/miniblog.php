...
if(mysql_num_rows($result) > 0) {
	while($posts = mysql_fetch_array($result)) {
		$vars = array(
			...
			'$posttime$'         => date('H:i', $posts['date']),
			'$postdateoriginal$' => date('Y-m-d', $posts['date']),
			'$postdate$'         => date($config['date-format'], $posts['date']),
			'$postcontent$'      => wpautop(stripslashes($posts['post_content'])),
		);
		
		$template_vars		= array_keys($vars);
		$template_values	= array_values($vars);
		
		$output = file_get_contents(PATH . 'themes/' . $config['theme-slug'] . '/template.html');
