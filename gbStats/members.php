...
if (isset($HTTP_COOKIE_VARS['adminp']) == "$adminpass" && $HTTP_COOKIE_VARS['adminu'] == "$adminuser") $adminlogged = 1;
...
if (isset($camp) || isset($page) || isset($domain)) {
...
while ($f = mysql_fetch_object($e)) {
	if ($f->title) {
		...
		if (isset($camp) && $camp == $f->id) $options .= " selected";
...
if (isset($allpages)) {
	foreach ($allpages as $key=>$value) {
		...
		if (isset($page) && $page == $key) $options2 .= " selected"; 
		$options2 .= ">$key</option>\n"; } }
if (isset($alldomains)) {
	foreach ($alldomains as $key=>$value) {
		$options3 .= "<option value=\"$key\""; 
		if (isset($_POST['domain']) && $_POST['domain'] == $key) $options3 .= " selected"; 
		$options3 .= ">$key</option>\n"; } }
...
if (isset($fday)&&$fmonth&&$fyear&&$tday&&$tmonth&&$tyear) {
	$filterdatetrue = 1;
...
if (!isset($filter)) echo "<tr><td class=showstats_header valign=top>This Month's Pageloads:</td><td class=showstats>$total_pageloads Pageloads $showalert2</td></tr>";
if (!isset($filter)) echo "<tr><td class=showstats_header valign=top>This Month's Pageloads:</td><td class=showstats>$total_pageloads Pageloads $showalert2</td></tr>";
if (!isset($filter)) echo "<tr><td class=showstats_header>Monthly Pageload Limit:</td><td class=showstats>$limitmessage2</td></tr>";
