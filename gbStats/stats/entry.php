...
echo "<font class=stats_header>Entry Pages</font><br><font class=stats_subheader>$filtershow<br>$filtershow2</font><p><hr><br>";





$c = mysql_query("select $stat as h from $statstableprefix"."_".$id." where $stat!='0' and ee='1' $filter $filter2 limit 1");
...
$c = mysql_query("select $stat as h,count(*) as g from $statstableprefix"."_".$id." where hit='1' and ee='1' $filter $filter2 group by $stat $order");
while ($d = mysql_fetch_object($c)) {
	...
	if ($d->h) {
		...
		if ($sofarcount >= $start && $sofarcount <= $end) {
			$echolater .= "<tr><td class=showstats width=10% align=left>$number.</td><td class=showstats width=80%><a href=\"$d->h\" title=\"$d->h\" target=_blank>$show</a></td><td class=showstats width=80%><a href=\"?stat=referer&page=$d->h\" title=\"\">Entered&nbsp;From</a></td><td class=showstats width=10% align=right>$d->g&nbsp;Entrances</td></tr>"; } }
...
echo "<br><img src=\"graph1.php?jptitle=Entry Pages (First 20)&jplegends=$graph_legends&jpdata1=$graph_data1\"><br><br>";
