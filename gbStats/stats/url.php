...
echo "<font class=stats_header>Popular Pages</font><br><font class=stats_subheader>$filtershow<br>$filtershow2</font><p><hr><br>";
...
while ($d = mysql_fetch_object($c)) {
	...
	if ($d->h) {
		...
		if ($sofarcount >= $start && $sofarcount <= $end) {
			$echolater .= "<tr><td class=showstats width=10% align=left>$number.</td><td class=showstats width=80%><a href=\"$d->h\" title=\"$d->h\" target=_blank>$show</a></td><td class=showstats width=10% align=right>$d->g&nbsp;Pageloads</td></tr>"; } }
...
echo "<br><img src=\"graph1.php?jptitle=Popular Pages (First 20)&jplegends=$graph_legends&jpdata1=$graph_data1\"><br><br>";
