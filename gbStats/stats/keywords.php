<?php 
 ...
 echo "<font class=stats_header>Keyword Activity</font><br><font class=stats_subheader>$filtershow<br>$filtershow2</font><p><hr><br>";
 ...
 $c = mysql_query("select $stat as h,url as k,engine as m,count(*) as g from $statstableprefix"."_"."$id where id>='1' $filter $filter2 group by $stat $order");
 while ($d = mysql_fetch_object($c)) {
 	if ($d->h) {
		$extra2 = $d->m;
		if (strlen($d->k) >= 55) $show = substr($d->k,0,55)."...";
		else $show = $d->k;
 		$number++;
 		if ($sofarcount >= $start && $sofarcount <= $end) {
 			$echolater .= "<tr><td class=showstats width=5% align=left valign=top>$number.</td><td class=showstats>$d->h&nbsp;($extra2)<br><a href=\"$d->k\" title=\"Landing Page: $d->k\">$show</a></td><td class=showstats width=5% align=right valign=top>$d->g&nbsp;Referrals</td></tr>"; } }
 ...
 echo "<br><img src=\"graph1.php?jptitle=Keyword Referrals (First 20)&jplegends=$graph_legends&jpdata1=$graph_data1\"><p>";
