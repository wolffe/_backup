...
echo "<font class=stats_header>Current Online Visitors</font><br><font class=stats_subheader>$filtershow<br>$filtershow2</font><p><hr><br>";


$c = mysql_query("select $stat as h from $statstableprefix"."_".$id." where $stat!='0' and $stat!='' and time>='$xyz' $filter $filter2 limit 1");
...
$c = mysql_query("select $stat as h,countrylanguage as j,count(*) as g from $statstableprefix"."_"."$id where id>='1' and time>='$xyz' $filter $filter2 group by $stat $order");
while ($d = mysql_fetch_object($c)) {
	if ($d->h) {
		...
		if ($sofarcount >= $start && $sofarcount <= $end) {
			...
			$extra2 .= "</td><td class=showstats width=10% align=right><a href=\"?stat=ip&ip=$d->h\" title=\"View Report for $d->h\">Activity/Path</a>";
