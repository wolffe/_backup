...
if(isset($_GET['action'])) {
	...
	elseif($action == 'edit') {
		...
		$myrow = mysql_fetch_array($sql);
		?>
		<form method="post" action="index.pending.php?id=<?php echo $myrow['id']?>">
...
$result = mysql_query("SELECT * FROM data WHERE approved = '0'");

while($row = mysql_fetch_array($result)) {
	echo '<tr>';
	echo '<td><small>';
	echo '<a href="index.pending.php?id='.$row['id'].'&amp;action=view">View</a> ';
	echo '<a href="index.pending.php?id='.$row['id'].'&amp;action=approve">Approve</a> ';
	echo '<a href="index.pending.php?id='.$row['id'].'&amp;action=edit">Edit</a> ';
	echo '<a href="index.pending.php?id='.$row['id'].'&amp;action=delete">Delete</a>';
	echo '</small></td>';
