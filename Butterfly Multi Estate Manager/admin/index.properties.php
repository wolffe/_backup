...
if(isset($_GET['action'])) {
	...
	if($action == 'unapprove') {
		$sql = "UPDATE data SET approved='0' WHERE id=".$_GET['id']." LIMIT 1";
		$result = mysql_query($sql);
		echo '<div class="confirm">Property ('.$_GET['id'].') unapproved!</div>';
	}
	...
	elseif($action == 'edit') {
		...
		$myrow = mysql_fetch_array($sql);
		?>
		<form method="post" action="index.properties.php?id=<?php echo $myrow['id']?>">
...
if(isset($_POST['update'])) {
	...
	$sql = "UPDATE data SET id='$id',date='$date',type='$type',title='$title',price='$price',year='$year',location='$location',description='$description',expire='$expire',category='$category',contact='$contact',phone='$phone',email='$email',url='$url',approved='1' WHERE id=$id";
	$result = mysql_query($sql);
...
$result = mysql_query("SELECT * FROM data WHERE approved = '1'");

while($row = mysql_fetch_array($result)) {
	echo '<tr>';
	echo '<td><small>';
	echo '<a href="index.properties.php?id='.$row['id'].'&amp;action=view">View</a> ';
	echo '<a href="index.properties.php?id='.$row['id'].'&amp;action=unapprove">Unapprove</a> ';
	echo '<a href="index.properties.php?id='.$row['id'].'&amp;action=edit">Edit</a> ';
	echo '<a href="index.properties.php?id='.$row['id'].'&amp;action=delete">Delete</a>';
	echo '</small></td>';
	echo '<td>'.$row['id'].'. <span class="type_title">'.$row['title'].'</span> <span class="type_content"><em>('.$row['category'].')</em></span> - <span class="type_date">'.$row['date'].'</span>/<span class="type_expire">'.$row['expire'].'</span> [<span class="type_user">'.$row['user'].'</span>]';
	echo '</td>';
	echo "</tr>";
}
