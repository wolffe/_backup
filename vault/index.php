...
/*
 * @file
 * VAULT - Ciprian Popescu <getbutterfly@gmail.com>
 *
 * Copyright (c) 2013, 2014, 2015 Ciprian Popescu
 *
 */

# Version information #
define('VERSION', '5.0'); // Current version of VAULT
define('INSTALL', 'EXPANDED');
define('MUTEX', $_SERVER['PHP_SELF']);

define('URLROOT', 'http://localhost:8888/vault');
      ...
      if ($_POST['type'] == 'file') {
		...
		elseif($_REQUEST['act'] == 'upload' && ft_check_upload() === TRUE && (LIMIT <= 0 || LIMIT > ROOTDIRSIZE)) {
			...
			for($i = 0; $i < count($_FILES['localfile']['name']); $i++) {
				$tmpFilePath = $_FILES['localfile']['tmp_name'][$i];
				if($tmpFilePath != '') {
					$newFilePath = ft_get_dir() . '/' . $_FILES['localfile']['name'][$i];
					if(!file_exists($newFilePath)) {
						if(move_uploaded_file($tmpFilePath, $newFilePath)) {
							chmod(ft_get_dir() . '/' . $_FILES['localfile']['name'][$i], PERMISSION);
							ft_set_message(t('!file was uploaded.', array('!file' => ft_get_nice_filename($_FILES['localfile']['name'][$i], 20))));
						}
					}
				}
			}
		...
		if(!is_array($files)) {
			...
			<table id="filelist">
				<thead>
					<tr>
						<th colspan="' . $colspan . '">
							<div class="head-manual">' . t('Files') . '</div>
							<form action="' . ft_get_self() . '" id="sort_form" method="get">
								<div>
									<select id="sort" name="sort">';
										...
										foreach($sorttypes as $k => $v) {
											$str .= '<option value="' . $k . '"'; if($sort == $k) $str .= ' selected'; $str .= '>' . $v . '</option>';
										}
									$str .= '</select>
				...
				if(count($files) <= 0)
					...
					if ($c['writeable'] && ft_check_fileactions() === TRUE) {
						$str .= "<td class='details'><span class='{$class}'><i class='fa fa-bars'></i></span><span class='hide' style='display:none;'><i class='fa fa-bars'></i></span></td>";
					} else {
				  ...
				  $plugin_data = implode('', ft_invoke_hook('filename', $c['name']));
					if ($c['type'] == "file"){
					  ...
					  $link = "<a>{$c['shortname']}</a>";
					  $sharelink = "<a href=\"".ft_get_dir()."/".rawurlencode($c['name'])."\" title=\"" .t('Show !file', array('!file' => $c['name'])). "\" target='_blank'><i class='fa fa-link'></i> share</a>";
					  if (HIDEFILEPATHS == TRUE) {
						...
						$str .= "
                        <td class='name'>{$link}{$plugin_data}</td>
                        <td class='name'>{$sharelink}</td>
                        <td class='size'>".ft_get_nice_filesize($c['size']);
						$countfiles++;
					} else {
						$str .= '
                        <td class="name">' . ft_make_link($c['shortname'], 'dir=' . rawurlencode($_REQUEST['dir']) . '/' . rawurlencode($c['name']), t('Show files in !folder', array('!folder' => $c['name']))) . $plugin_data . '<span class="sharelink">' . URLROOT . $_REQUEST['dir'] . '/' . $c['name'] . "/</span></td><td></td><td class='size'>{$c['size']} ".t('files');
						$countfolders++;
					}
...
function ft_make_footer() {
	return '
    <div id="footer" class="buffer">
        <p><b>VAULT</b> &copy;' . date('Y') . '</p>
    </div></div>';
}
...
function ft_make_header() {
    global $ft;
    $str = '<h1 id="title"><span>VAULT <i>&#59230;</i> </span>' . ft_make_link(t('Home'), '', t('Go to home folder')) . ' ';
    if(empty($_REQUEST['dir'])) {
        $str .= '/</h1>';
    } else {
        // Get breadcrumbs.
    ...
    if(LOGIN == TRUE) {
        ...
        $str .= ft_make_link(t("[logout]"), "act=logout", t("Logout")).'</p>';
...
function ft_make_login() {
    ...
    $str .= '<form action="' . ft_get_self() . '" method="post" id="loginbox">';
        ...
        $str .= '
        <div class="buffer-lite"><input type="text" size="25" name="ft_user" id="ft_user" placeholder="' . t('Username') . '"></div>
		<div class="buffer-lite"><input type="password" size="25" name="ft_pass" id="ft_pass" placeholder="' . t('Password') . '"></div>
        <div class="buffer-lite">
            <input type="submit" value="' . t('Login') . '" id="login_button">
        	...
        	<div class="section" id="create">
        		...
        		<form action="' . ft_get_self() . '" method="post" enctype="multipart/form-data">
        			<div id="uploadsection">
        				<input type="file" name="localfile[]" multiple>
        				<input type="hidden" name="act" value="upload">
        				<input type="hidden" name="dir" value="' . $_REQUEST['dir'] . '">
        			</div>
        			<div id="uploadbutton">
        				<input type="submit" name="submit" value="' . t('Upload File(s)') . '">
        			</div>
		...
		<div>
		  ...
		  <input type="radio" name="type" value="url" id="type-url" /> <label for="type-url">'.t('From URL').'</label>
			...
			<div><br>
				...
				<input type="hidden" name="dir" value="'.$_REQUEST['dir'].'" />
				<input type="submit" id="mkdirsubmit" name="submit" value="'.t('Create New').'" />
			</div>
		</form>
	</div>';
	}
