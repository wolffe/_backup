...
<?php if(is_authed()) {?>
	...
	if(isset($_POST['submit_category'])) {
		...
		$ssql = "SELECT * FROM items WHERE category = '$category' AND status = '1'";
		showresults($ssql, $rank);
	}
	else {
		?>
		<h2><?php echo $lang['VIEW'];?> <?php echo $lang['DB_UNEMPLOYED'];?></h2>
		<p class="helpbox"><?php echo $lang['SORTING_HELP'];?></p>
		<?php
		$ssql = "SELECT * FROM items WHERE status = '1' ORDER BY id ASC";
		showresults($ssql, $rank);
    }
}
?>
